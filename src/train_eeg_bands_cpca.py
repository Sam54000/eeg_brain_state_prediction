...
def main(config: "ModelConfig") -> None:
    ...
    print(f"Starting processing with task: {config.task}, description: {config.description}")
...
if __name__ == "__main__":
    ...
    additional_info = "All"
    for eeg, task, cpca in list(combination):

        config = ModelConfig(
            description = eeg+cpca,
            eeg_feature = eeg,
            additional_info=additional_info,
            data_directory=f"data/eeg_bands_cpca/{eeg}{additional_info}",
            task = task,
        ) 
        ...
        if eeg == "GfpBk":
            config.n_channels = 1
            config.n_bands = 1
            config.feature_set["eeg"]["channel"] = np.array([0])
            config.feature_set["eeg"]["band"] = np.array([0])

        elif eeg == "BandsGfpBk":
            n_channels = 1
            n_bands = 5
            config.feature_set["eeg"]["channel"] = np.array([0])
            config.feature_set["eeg"]["band"] = np.arange(5)

        elif eeg == "BandsEnvBk":
            n_channels = 61
            n_bands = 5
            config.feature_set["eeg"]["channel"]= (
                np.arange(n_channels).repeat(n_bands))
            config.feature_set["eeg"]["band"]= (
                np.tile(np.arange(n_bands),n_channels))
