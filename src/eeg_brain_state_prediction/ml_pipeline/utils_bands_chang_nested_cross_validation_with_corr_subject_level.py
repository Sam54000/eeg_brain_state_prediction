...
import eeg_brain_state_prediction.ml_pipeline.cross_correlation as cc
from sklearn.model_selection import train_test_split
import pandas as pd
import argparse
import eeg_brain_state_prediction.ml_pipeline.combine_data as combine_data
...
def nested_cross_val(train_arch: arch.BidsArchitecture,
    ...
    for i in range(iteration):
        features_subjects, train_subjects = train_test_split(
            train_arch.subjects,
            test_size = 0.5)
        features_arch = train_arch.select(subject = features_subjects)
        nested_train_arch = train_arch.select(subject = train_subjects)
        tstats = cc.calculate_sorted_tstat(
            features_arch,
            xcorr,
            xcorr_info,
            )
        ...
        for cap in config.caps:
            sorted_stats = tstats[tstats["ts_CAPS"] == cap].sort_values(
                by = "stats",
                ascending=False,
            )
            for n_features in range(1,config.nb_desired_features+1):
                chan = sorted_stats["electrode"].values[:n_features]
                band = sorted_stats["frequency_Hz"].values[:n_features]
                X_train, Y_train, X_test, Y_test = process_single_iteration(
                    cap = cap,
                    big_data=big_data,
                    train_keys=nested_train_arch.database.index,
                    test_keys=[test_index],
                    feature_set={
                        "eeg":{
                            "channel": chan,
                            "band": band,
                ...
                results["subject"].append(test_arch["subject"])
                results["session"].append(test_arch["session"])
                results["acquisition"].append(test_arch["acquisition"])
                results["run"].append(test_arch["run"])
...
def pipeline(
    ...
    xcorr_results, xcorr_info = cc.population_corr(architecture=architecture,
                                       big_data=big_data)
    ...
    for test_keys, test_session in test_arch:
        results = nested_cross_val(
            train_arch=train_arch,
            test_arch=test_session,
            test_index=test_keys,
            iteration = 5,
            config=config,
            big_data=big_data,
            xcorr=xcorr_results,
            xcorr_info=xcorr_info,
        )
