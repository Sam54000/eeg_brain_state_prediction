...
def make_saving_path(config: "ModelConfig", subject: str) -> Path:
    ...
    filename = f"sub-{subject}_ses-{config.session}_task-{config.task}_desc-{config.description}{config.additional_info}_predictions.csv"
...
def pipeline(
    ...
    test_arch.select(session = config.session)
    results = initialize_results_dict()
    features_csv = pd.read_csv(f"/home/slouviot/01_projects/eeg_brain_state_prediction/data/custom_envelope_caps/group_level/sub-{subject}_task-{config.task}_desc-CustomEnvBk_predictions.csv")
    for n_features in config.nb_desired_features:
        ...
        test_keys = test_arch.database.index.values

        for cap in config.caps:
            try:
                best_n_features = get_best_n_feature_combinations(
                    n_features=n_features,
                    aggregated_selection=features_csv[
                        (features_csv["ts_CAPS"] == cap) & (features_csv["session"] != int(config.session))],
                    to_sort = "pearson_r",
                )
                config.feature_set |= best_n_features
