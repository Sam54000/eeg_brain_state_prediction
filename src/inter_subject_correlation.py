...
import pyriemann
#from scipy.sparse.lil_matrix import diagonal as sp_diag

def calc_corrca(epochs, times, mask = None, riemann = True, **par):
    """
    ...
    **par : dict
    ...
    W, ISC, A = fit(X, gamma=par['gamma'], k=par['K'], mask = mask, riemann=riemann)
    ...
    return W, ISC, A, Y, Yfull

##################
# MAIN FUNCTIONS #
##################
def fit(X, version=2, gamma=0, k=None, mask = None, riemann = True):
    '''
    ...
    if mask is not None:
        for n in range(N):
            temp.append(np.cov(X[n,:,mask[n,:]].T))
    ...
    Rb = (Rt - Rw) / (N-1)
    print(f"Rw: {Rw.shape}, Rt: {Rt.shape}, Rb: {Rb.shape}")
    ...
    if k < D:
        ...
        invR = regInv(Rw, k)
        print(f"Inversed Rw: {invR.shape}")
...
if __name__ == "__main__":
    ...
    root_directory = "/data2/Projects/eeg_fmri_natview/derivatives/"
    ...
    data = list()
    length_time = list()
    for filename in selector.layout:
        file_desc = filename[filename.find('desc-')+5:filename.find('_eeg')]
        if file_desc == 'rawBlinksRemoved':
            with open(filename, 'rb') as file:
                indiv_dat = pickle.load(file)
            length_time.append(indiv_dat['feature'].shape[1])
            data.append(indiv_dat['feature'])
    min_shape = min(length_time)
    for idx, array in enumerate(data):
        data[idx] = array[:,:min_shape]
        
    times = indiv_dat['time']
    data = np.stack(data)
    W, ISC, A, Y, Yfull, ISC_thr = calc_corrca(data,times,
