...
class EEGfeatures:
    def __init__(self, raw: mne.io.Raw):
        self.raw = raw.resample(250)
        self.channel_names = raw.info["ch_names"]
        self.frequencies = list()
        self.croping_values = specific_crop(raw, margin=0, return_time=True)
    ...
    def _extract_envelope(
        ...
        for band in frequencies:
            ...
            envelope = (
                filtered.copy().apply_hilbert(envelope=True).crop(*self.croping_values)
            )
    ...
    def extract_custom_band_envelope(
        self: "EEGfeatures",
        highest_frequency: int = 40,
        lowest_frequency: int = 1,
        frequency_step: int = 1,
...
def individual_process(
    ...
    print("\nPROCESSING:")
    bids_path = BIDSPath(**file_entities, root=derivatives_path, datatype="eeg")
