...
base_data_dir = os.path.join(base_dir,'data_prep', 'prediction_model_data_eeg_features_v2')
data_dir = os.path.join(base_data_dir, f"dictionary_group_data_Hz-{predHz}")

base_out_dir = os.path.join(base_dir, "analysis3")
out_dir = os.path.join(base_out_dir, f"prediction_results_eeg_features-mean_Hz-{predHz}")
...
for test_task in tasks:
    for test_bstate in brainstates:
        pred_results_dir = os.path.join(out_dir, f"task-{test_task}_bstate-{test_bstate}_nfeats-{n_feat}")
        ...
        for key in data_eyetracking.keys():
            print(f"===============================================")    
            print(f"- {key}")
            ...
            EEG_FEATURES = the_eeg_features[key][0]
            EEG_FEATURES = np.nanmean(EEG_FEATURES, axis=0)
            #EEG_FEATURES = EEG_FEATURES[19, :, :]
            ...
            EEG_FEATURES = EEG_FEATURES[start_index:]
            fmri_ts_mask = fmri_ts_mask[start_index:]
            
            PREDICTION_FEATURES = EEG_FEATURES
            PREDICTION_FEATURES = zscore(PREDICTION_FEATURES).T
            #PREDICTION_FEATURES = PREDICTION_FEATURES.T
            #PREDICTION_FEATURES = zscore(PREDICTION_FEATURES.T).T
            #plt.plot(PREDICTION_FEATURES[1:,:].T)

            #plt.plot(zscore(FMRI_TARGET_DATA))
            #plt.plot(PREDICTION_FEATURES.T)
            ...
            SUBJECT_KEYS = np.hstack([SUBJECT_KEYS, tmp_keys]) if SUBJECT_KEYS.size else tmp_keys
            STACKED_TARGETS = np.hstack([STACKED_TARGETS, FMRI_TARGETS]) if STACKED_TARGETS.size else FMRI_TARGETS
            STACKED_TARGETS_MASK = np.hstack([STACKED_TARGETS_MASK, FMRI_TARGETS_MASK]) if STACKED_TARGETS_MASK.size else FMRI_TARGETS_MASK
            for ii in np.arange(n_feat,n_ts,1):
                X = np.reshape(PREDICTION_FEATURES[:, (ii-n_feat):(ii)].flatten(),(1,-1))
                STACKED_FEATURES = np.vstack([STACKED_FEATURES, X]) if STACKED_FEATURES.size else X     
                tmp_feature_mask = FEATURE_MASK[(ii-n_feat):(ii)]
                STACKED_FEATURES_MASK = np.vstack([STACKED_FEATURES_MASK, tmp_feature_mask]) if STACKED_FEATURES_MASK.size else tmp_feature_mask     
        # --- STACK HERE -------------------------------------------------------------------------
        ...
        for test_sub in subjects:
            for test_ses in sessions:
                plt.close('all')
                print(f"- {test_sub} - {test_ses} - {test_task} - {test_bstate}")
                print(f"===============================================")
                test_key = f"sub-{test_sub}_ses-{test_ses}_task-{test_task}"
                ...
                yticks_labels = np.tile(iter_freqs_labels,1)
